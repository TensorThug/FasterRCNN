...
class _COCOMeta(object):
    ...
    def create(self, cat_ids, cat_names):
        ...
        assert len(cat_ids) == (cfg.DATA.NUM_CATEGORY - 1) and len(cat_names) == (cfg.DATA.NUM_CATEGORY - 1)
        self.cat_names = cat_names
        self.class_names = ['BG'] + self.cat_names

        # background has class id of 0
        self.category_id_to_class_id = {cat_ids[cat_names.index('car')]: 2, cat_ids[cat_names.index('person')]: 3}
...
COCOMeta = _COCOMeta()
valid_cat_names = ['car', 'person']  # categories: 'person'(=1), 'car'(=3)
...
class COCODetection(object):
    def __init__(self, basedir, name):
        ...
        # initialize the meta
        cat_ids = self.coco.getCatIds(catNms=valid_cat_names)
    ...
    def load(self, add_gt=True, add_mask=False):
        ...
        with timed_operation('Load Groundtruth Boxes for {}'.format(self.name)):
            img_ids = self.coco.getImgIds(catIds=self.coco.getCatIds(catNms=valid_cat_names))
            img_ids.sort()
    ...
    def load_img_annotations(self, imID, add_gt=True, add_mask=False):
        """
        Args:
        ...
        # dict containing: height,width,id,file_name
        img = self.coco.loadImgs(imID)
        if not img:  # if img is an empty list => pass
            pass
        else:
            img = img[0]  # extract the first (and only) dictionary from the list
            self._use_absolute_file_name(img)
            if add_gt:
                self._add_detection_gt(img, add_mask)
            img = [img]
    ...
    def _add_detection_gt(self, img, add_mask):
        ...
        valid_cats = self.coco.getCatIds(catNms=valid_cat_names)
        # clean-up boxes
        valid_objs = []
        width = img['width']
        height = img['height']
        for obj in objs:
            if obj.get('ignore', 0) == 1:
                continue
            if not(obj['category_id'] in valid_cats):
                continue
            x1, y1, w, h = obj['bbox']
    ...
    @staticmethod
    def load_img_from_many(dict_coco_annt, imID, add_gt=True, add_mask=False):
        """
        Load and merges several instance files together.
        Returns the same format as `COCODetection.load_img_annotations`.
        """
        ret = []
        for key in dict_coco_annt:
            ret.extend(dict_coco_annt[key].load_img_annotations(imID, add_gt, add_mask=add_mask))
        return ret
    ...
    def load_annt(basedir, names):
        ...
        dict_coco_annt = {}
        for n in names:
            dict_coco_annt[n] = COCODetection(basedir, n)
        return dict_coco_annt
