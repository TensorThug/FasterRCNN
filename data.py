...
from KITTI_for_koby import KITTIDetection
...
def get_train_dataflow():
    ...
    """
    # Class names:
    # cfg.DATA.CLASS_NAMES = ['BG', 'tank', 'car', 'person']


    # Load data:
    basedir = {'coco': r'/media/koby_a/Data/databases/coco', 'kitti': r'/media/koby_a/Data/databases/KITTI/training',
               'open_images': r'/media/koby_a/Data/databases/open_images'}
    names = {'coco': ['train2014'], 'kitti': ['labels'], 'open_images': ['train']}

    imgs = []
    imgs_coco = COCODetection.load_many(basedir['coco'], names['coco'], add_gt=True, add_mask=False)
    imgs.extend(imgs_coco)
    imgs_kitti = KITTIDetection.load_many(basedir['kitti'], names['kitti'], add_gt=True, _load_from_json_file=True)
    imgs.extend(imgs_kitti)
    imgs_open_images = OpenImagesDetection.load_many(basedir['open_images'], names['open_images'],
                                                     add_gt=True, _load_from_json_file=True)
    imgs.extend(imgs_open_images)
...
def get_eval_dataflow(db_name):
    basedir = cfg.VAL_DATA.BASEDIR
    names = cfg.VAL_DATA.NAMES

    if db_name == 'coco':
        imgs = COCODetection.load_many(basedir[db_name], names[db_name], add_gt=False)
    elif db_name == 'open_images':
        imgs = OpenImagesDetection.load_many(basedir[db_name], names[db_name], add_gt=False)
