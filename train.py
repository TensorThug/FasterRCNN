...
from coco_for_koby import COCODetection
from open_images_for_koby import OpenImagesDetection
...
def offline_evaluate(pred_func, output_file):
    valid_db = ['coco', 'open_images']
    load_annt_func = {'coco': COCODetection.load_annt, 'open_images': OpenImagesDetection.load_annt}
    load_img_func_dict = {'coco': COCODetection.load_img_from_many,
                          'open_images': OpenImagesDetection.load_img_from_many}
    basedir = cfg.VAL_DATA.BASEDIR
    names = cfg.VAL_DATA.NAMES
    ...
    for db in valid_db:
        df = get_eval_dataflow(db)
        prediction_results[db], fp[db], db_size[db] = eval_db(df, lambda img: detect_one_image(img, pred_func),
                                                              load_annt_func[db], basedir[db], names[db],
                                                              load_img_func_dict[db])
    eval_results(output_file, prediction_results, fp, db_size, valid_db)
...
def check_anything_you_want(pred_func, input_file):
    img = cv2.imread(input_file, cv2.IMREAD_COLOR)
    pred = lambda img: detect_one_image(img, pred_func)
    results = pred(img)


def freeze_graph_after_train(ckpt_dir, output_names):
    output_names = ['tower-pred-0/' + name + ', ' for name in output_names]
    str_output_names = ''.join(output_names)
    str_output_names = str_output_names[:-2]
    input_graph_path = ckpt_dir + '/original.pbtxt'
    input_ckpt_path = tf.train.latest_checkpoint(checkpoint_dir=ckpt_dir).split(".")[0]
    output_graph_path = ckpt_dir + '/frozen.pb'
    freeze_graph.freeze_graph(
        ...
        output_node_names=str_output_names,
...
class EvalCallback(Callback):
    ...
    def _setup_graph(self):
        ...
        self.df = get_eval_dataflow('coco')
    ...
    def _eval(self):
        ...
        fp = {}
        db_size = {}
        pred_res['coco'], fp['coco'], db_size['coco'] = eval_db(self.df, lambda img: detect_one_image(img, self.pred),
                                                                COCODetection.load_annt, cfg.VAL_DATA.BASEDIR['coco'],
                                                                cfg.VAL_DATA.NAMES['coco'],
                                                                COCODetection.load_img_from_many)
        output_file = os.path.join(
            logger.get_logger_dir(), 'outputs{}.txt'.format(self.global_step))
        eval_results(output_file, pred_res, fp, db_size, 'coco')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', help='log directory', default='/media/koby_a/Data/train_log/test')
    parser.add_argument('--visualize', action='store_true', help='visualize intermediate results')
    parser.add_argument('--evaluate', help="Run evaluation. "
                                           "This argument is the path to the output txt evaluation file")
    parser.add_argument('--predict', help="Run prediction on a given image. "
                                          "This argument is the path to the input image file")
    parser.add_argument('--config', help="A list of KEY=VALUE to overwrite those defined in config.py",
                        nargs='+')
    parser.add_argument('--check', help="Added by koby - check implementation of different functions")
    ...
    if args.visualize or args.evaluate or args.predict or args.check:
        assert args.load
        finalize_configs(is_training=False)

        if args.predict or args.visualize or args.check:
        ...
        if args.visualize:
            ...
            if args.evaluate:
                assert args.evaluate.endswith('.txt'), args.evaluate
                offline_evaluate(pred, args.evaluate)
            elif args.predict:
                predict(pred, args.predict)
            elif args.check:
                check_anything_you_want(pred, args.check)
    else:
        ...
        traincfg = TrainConfig(
            ...
            max_epoch=cfg.TRAIN.LR_SCHEDULE[-1] * factor // stepnum,  # 500,
        ...
        # Clear the current graph and freeze the trained graph
        output_names = MODEL.get_inference_tensor_names()[1]
        tf.reset_default_graph()
        freeze_graph_after_train(args.logdir, output_names)
