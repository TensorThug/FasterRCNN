...
class ModelSaver(Callback):
    ...
    def _trigger(self):
        try:
            ...
            # Koby - create proto files:
            tf.train.write_graph(tf.get_default_graph().as_graph_def(), self.checkpoint_dir, name='original.pbtxt', as_text=True)
            tf.train.write_graph(tf.get_default_graph().as_graph_def(), self.checkpoint_dir, name='original.pb', as_text=False)
